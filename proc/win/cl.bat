...
rem Link options
set l_options=/EHsc /link /SUBSYSTEM:WINDOWS /NODEFAULTLIB:msvcrt.lib

rem Compile Release
rem cl /MP /FS /Ox /W0 /Fe%name%.exe %src_all% %inc% ^
rem /EHsc /link /SUBSYSTEM:WINDOWS /entry:mainCRTStartup /NODEFAULTLIB:msvcrt.lib /NODEFAULTLIB:LIBCMT ^
rem %os_libs%
